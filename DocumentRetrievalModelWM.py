...
class DocumentRetrievalModelWM:
    def __init__(self,paragraphs, model, removeStopWord = False,useStemmer = False):
    ...
    def getTermFrequencyCount(self,paragraph):
        ...
        # TODO: Iterate through the words in each sentences
        # HINT: use "word_tokenize" to break down sentence into words 

        # TODO: Convert the word to lowercase

        # TODO: Perform stopword removal, if the argument for stopword removal is set to True
        
        # TODO: Increment the frequency count for each word in "wordFrequency" dictionary 

        # An example of "wordFrequency" dictionary:
        # {
        #     "marvel": 2,
        #     "iron": 1
        # }
    ...
    def computeEmbedding(self):
        ...
        for index in range(0,len(self.paragraphs)):
            ...
            # TODO: Update the "getTermFrequencyCount"
            ...
            # Task: Average the "paraVector" by the number of words found in the paragraph

            # TODO: Append the "paraVector" into paragraphVectors
        
        self.paragraphVectors = np.array(self.paragraphVectors)
    ...
    def getSimilarParagraph(self,queryVector):
        pRanking = []
        similarity_matrix = cosine_similarity(queryVector.reshape(1, -1), self.paragraphVectors)

        np.fill_diagonal(similarity_matrix, 0)
        similarities = similarity_matrix[0]

        top3 = np.flip(similarities.argsort()[-3:][::1])

        return [(k, 0) for k in top3]
        # for index, paraVector in enumerate(self.paragraphVectors):
        #     sim = cosine_similarity(queryVector, paraVector)
